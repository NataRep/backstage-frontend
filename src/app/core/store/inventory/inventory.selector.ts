...
export const selectInventoryByCategory = (category: string | null) =>
  createSelector(selectAllInventory, (inventory) => {
    if (!category) return inventory;
    return inventory.filter((item) => item.category === category);
  });

export const selectInventoryEntities = createSelector(selectAllInventory, (inventory) =>
  inventory.reduce(
    (acc, item) => {
...
export const selectInventoryItemById = (id: string) =>
  createSelector(selectInventoryEntities, (entities) => entities[id] || null);

export const selectInventoryGroupedByCategory = (category: string | null) =>
  createSelector(selectAllInventory, (allInventory) => {
    const filtered = category ? allInventory.filter((i) => i.category === category) : [];
