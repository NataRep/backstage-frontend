...
export class EmployeeFacade {
  ...
  createEmployee(person: Person, employmentData: WorkerBase) {
    ...
    return forkJoin({
      person: this.personService
        ...
        .pipe(
          catchError((err) =>
            err.code === 'PERSON_ALREADY_EXISTS'
      ...
      worker: from(this.workersService.getByPersonId(person.personId)).pipe(
        switchMap((existing) =>
          existing
            ? of(existing)
            : from(
                this.workersService.create({ ...employmentData, personId: person.personId })
  ...
  updateWorkerEmployeeProfile(worker: Worker): Observable<Worker> {
    return from(this.workersService.update(worker.id!, worker)).pipe(map(() => worker));
  }
      ...
      exhaustMap(() =>
        from(this.workersService.getByPersonId(id)).pipe(
          switchMap((worker) => {
            ...
            return from(this.workersService.delete(worker.id)).pipe(map(() => id));
          })
        )
      )
    );
    ...
    return this.workersService.subscribeAllActiveEmployees().pipe(
      switchMap((workers) => {
        ...
        return this.personService
          .getAllEmployees()
          .pipe(map((persons) => this.mapToEmployeeProfiles(persons, workers)));
      })
    );
  }
