...
export class WorkerDataService {
  ...
  subscribeAll(constraints: QueryConstraint[] = []): Observable<WithId<Worker>[]> {
    return this.firebase
      .subscribeCollection<Worker>(this.collectionName, constraints)
      .pipe(retry(this.retryConfig));
  }

  subscribeAllActiveEmployees(): Observable<WithId<Worker>[]> {
    return this.firebase
      .subscribeCollection<Worker>(this.collectionName, [where('isActive', '==', true)])
      .pipe(retry(this.retryConfig));
  }

  subscribeOne(id: string): Observable<WithId<Worker> | null> {
    return this.firebase
      .subscribeDoc<Worker>(this.collectionName, id)
      .pipe(retry(this.retryConfig));
  }

  subscribeChanges(
    constraints: QueryConstraint[] = []
  ): Observable<{ type: 'added' | 'modified' | 'removed'; doc: WithId<Worker> }[]> {
    return this.firebase
      .subscribeCollectionChanges<Worker>(this.collectionName, constraints)
      .pipe(retry(this.retryConfig));
  }
}
