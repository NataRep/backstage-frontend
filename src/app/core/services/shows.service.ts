...
export class ShowsService {
  ...
  subscribeAll(constraints: QueryConstraint[] = []): Observable<ShowItem[]> {
    return this.firebase
      .subscribeCollection<ShowItem>(this.collectionName, constraints)
      .pipe(retry(this.retryConfig));
  }

  subscribeChanges(
    constraints: QueryConstraint[] = []
  ): Observable<{ type: 'added' | 'modified' | 'removed'; doc: ShowItem }[]> {
    return this.firebase
      .subscribeCollectionChanges<ShowItem>(this.collectionName, constraints)
      .pipe(retry(this.retryConfig));
  }
}
